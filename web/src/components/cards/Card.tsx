...
export function CardFace(props: CardFaceProps) {
  ...
  return (
    <div
      class={cn(
        "card-face absolute inset-0 overflow-hidden rounded-[12px] bg-[#fdf9f0]",
        local.class,
      )}
    >
...
export function Card(props: CardProps) {
  ...
  return (
    <div
      ...
      role="img"
      aria-label={
        local.faceDown || !local.rank || !local.suit
          ? "face-down card"
          : `${local.rank} of ${({ s: "spades", h: "hearts", d: "diamonds", c: "clubs" } as const)[local.suit]}`
      }
      {...others}
    >
      <Show
        when={!local.faceDown && local.rank && local.suit}
        fallback={<CardBack size={local.size} />}
      >
        <CardFace
          rank={local.rank!}
          suit={local.suit!}
...
export function CardBack(props: VariantProps<typeof cardVariants> & { class?: string }) {
  ...
  return (
    <div
      class={cn(
        "card-back absolute inset-0 overflow-hidden rounded-[12px] bg-[#a8103f]",
        local.class,
      )}
    >
      <BackArt />
