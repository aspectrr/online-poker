    ...
    <div class="pointer-events-none absolute inset-0 flex flex-col items-center justify-center gap-3">
      ...
      <Show when={t().potCents > 0 || boards()[0].length > 0}>
        <div class="flex items-center gap-2 rounded-full border border-accent/25 bg-black/35 px-3.5 py-1 shadow-lg shadow-black/30 backdrop-blur-[2px]">
          <ChipStack stack={t().potCents} unit={Math.max(1, t().bbCents)} max={12} size={11} class="flex-row-reverse items-center" />
        ...
        {(cards, row) => (
          <div
            class={cn(
              ...
              t().isDoubleBoard && t().boardWins.includes(row) && 'animate-board-win ring-2 ring-success/80',
            )}
          >
            <Show when={isDouble()}>
              <span
              ...
              >
                ...
                <Show when={t().potCents > 0}>
                  <span class="font-semibold tabular-nums normal-case text-accent/90">½ {money(halfPot())}</span>
                </Show>
              </span>
            </Show>
                ...
                {(card, i) => (
                  <div class="animate-deal" style={dealDelay(i(), 110)}>
                    <CardRow
                      ...
                      size="sm"
                      revealed
                      class={cn('shadow-lg shadow-black/40', card.rabbit && 'opacity-55 -rotate-6 saturate-50')}
                    />
                  </div>
                )}
              </For>
