...
export function ActionBar(props: {
  ...
  createEffect(on(
    ...
    (min) => {
      if (min != null) { setRaiseTo(min); setRaising(false); setTyped(''); setTyping(false) }
    },
  ))
  ...
  const commitRaise = () => {
    ...
    setRaising(false); setTyping(false); setTyped('')
  }
  ...
  const primary = () => {
    ...
    return { label: `Call ${money(la.callCents)}`, sub: `C`, action: { kind: 'call' } as PlayerAction }
  }
  ...
  const onKey = (e: KeyboardEvent) => {
    ...
    switch (e.key.toLowerCase()) {
      case 'f':
        if (la.canFold) { e.preventDefault(); props.send({ kind: 'fold' }) }
        break
      case 'c':
        e.preventDefault()
        ...
        break
      case 'r':
        if (la.canRaise) { e.preventDefault(); setRaising(true); inputEl()?.focus() }
        break
      case 'enter':
        e.preventDefault()
        ...
        break
      case 'escape':
        if (raising() || typing()) { e.preventDefault(); setRaising(false); setTyping(false); setTyped('') }
        break
      case 'arrowright':
      case 'arrowup': {
        ...
        e.preventDefault()
        setRaising(true)
        setRaiseTo((v) => Math.min(la.maxRaiseToCents, v + (e.shiftKey ? stepCents() * 2 : stepCents())))
        break
      }
      case 'arrowleft':
      case 'arrowdown': {
        ...
        e.preventDefault()
        setRaising(true)
        setRaiseTo((v) => Math.max(la.minRaiseToCents, v - (e.shiftKey ? stepCents() * 2 : stepCents())))
        break
      }
    }
  }
              ...
              <div class="relative">
                <Input
                  ...
                  onKeyDown={(e) => {
                    ...
                    if (e.key === 'Escape') { setTyping(false); setTyped(''); inputEl()?.blur() }
                  }}
                />
