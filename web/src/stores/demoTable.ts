...
function scriptHand(button: number, handNo: number): Step[] {
  const sb = (button + 1) % MAX,
    bb = (button + 2) % MAX,
    utg = (button + 3) % MAX;
  const mp = (button + 4) % MAX,
    co = (button + 5) % MAX;
  const v = (
    seat: number,
    kind: Step extends never ? never : any,
    label: string,
    extra: Partial<Extract<Step, { t: "villain" }>> = {},
  ): Step => ({ t: "villain", seat, kind, label, ...extra }) as Step;
  return [
  ...
  function send(action: PlayerAction) {
    ...
    if (action.kind === "raise") {
      ...
      if (to < la.minRaiseToCents || to > la.maxRaiseToCents) {
        setErr(
          "lastError",
          `Raise between ${fmt(la.minRaiseToCents)} and ${fmt(la.maxRaiseToCents)}`,
        );
        later(() => setErr("lastError", null), 2500);
        return;
      }
