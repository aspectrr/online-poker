...
export const cardText = (cs: Card[] | undefined): string => uiCards(cs).map((c) => c.rank + SUIT_CHAR[c.suit]).join(' ')
...
export function actionLabel(kind: string, streetBet: number): string {
  switch (kind) {
    ...
    case 'raise': return streetBet > 0 ? `Raise to ${(streetBet / 100).toFixed(2)}` : 'Raise'
    case 'sb': return 'SB'
    case 'bb': return 'BB'
