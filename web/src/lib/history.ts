...
export function handTotals(row: HandRow): {
  potCents: number;
  winners: { player: string; amountCents: number }[];
} {
...
export function reviewHand(row: HandRow): HandReview {
  ...
  for (const ev of d.events ?? []) {
    ...
    switch (ev.type) {
      case "blinds_posted":
        lines().push({
          player: ev.player ?? "",
          text: `posts ${++blindNo <= 2 ? (blindNo === 1 ? "SB" : "BB") : "blind"} ${cents(ev.amount)}`,
        });
        break;
      case "antes_posted":
        ...
        const text =
          k === "fold"
            ...
            : k === "check"
              ? "checks"
              : k === "call"
                ? `calls ${cents(ev.amount)}`
                : k === "raise"
                  ? `raises to ${cents(ev.to || ev.amount)}`
                  : (k ?? "acts");
  ...
  const streets: StreetLog[] = [...byStreet]
    .toSorted((a, b) => streetOrder.indexOf(a[0]) - streetOrder.indexOf(b[0]))
  ...
  const boardRows: BoardRow[] = [...boards].map(([idx, streetLogs]) => ({
    label:
      boards.size > 1
        ? d.bomb_pot
          ? `Board ${idx === 0 ? "A" : "B"}`
  ...
  return {
    ...
    holes: (d.holes ?? []).map((h) => ({
      seat: h.seat,
      player: nameOf(h.seat),
      cards: (h.cards ?? []).map(toUICard),
    })),
    streets,
    boardRows,
    awards,
