...
let p = raisePresets({
  ...
  bbCents: 20,
  minRaiseToCents: 40,
  maxRaiseToCents: 5010,
});
assert(
  p.some((x) => x.label === "2.5bb" && x.toCents === 50),
  `2.5bb→50, got ${JSON.stringify(p)}`,
);
assert(
  p.some((x) => x.label === "3.5bb" && x.toCents === 70),
  "3.5bb→70",
);
...
p = raisePresets({
  ...
  bbCents: 20,
  minRaiseToCents: 100,
  maxRaiseToCents: 5000,
});
assert(
  p.some((x) => x.label === "3x" && x.toCents === 180),
  `3x→180, got ${JSON.stringify(p)}`,
);
...
assert(
  p.some((x) => x.label === "33%" && x.toCents === 33),
  `33%→33, got ${JSON.stringify(p)}`,
);
assert(
  p.some((x) => x.label === "100%" && x.toCents === 100),
  "100%→100",
);
...
assert(
  p.some((x) => x.label === "50%" && x.toCents === 125),
  `50%→125, got ${JSON.stringify(p)}`,
);
assert(
  p.some((x) => x.label === "75%" && x.toCents === 163),
  "75%→163",
);
